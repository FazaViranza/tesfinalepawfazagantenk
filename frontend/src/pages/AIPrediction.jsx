...
export default function AIPrediction() {
  ...
  const chartData = data ? [
    ...data.historicalData.map(d => ({ date: d.date.slice(5), historical: d.revenue, type: 'Historis' })),
    ...data.forecastData.map(d => ({ date: d.date.slice(5), forecast: d.predictedRevenue, orders: d.predictedOrders, conf: d.confidence })),
  ] : [];
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-gray-400 mt-0.5">Proyeksi permintaan berbasis time-series & trend regression</p>
        </div>
        ...
        <>
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            {[
              ...
              { label: 'Rata-rata Harian', value: formatRp(data.summary.avgDailyRevenue), sub: 'Berdasarkan 30 hari historis', color: 'green' },
              ...
              <div key={s.label} className="bg-gray-900 border border-gray-800 rounded-2xl p-4">
                ...
                <p className={`text-xl font-bold mt-1 ${s.color === 'red' ? 'text-red-400' : s.color === 'green' ? 'text-green-400' : 'text-indigo-300'}`}>{s.value}</p>
              ...
              <table className="w-full text-sm">
                <thead>
                  <tr className="border-b border-gray-800 bg-gray-800/50">
                    ...
                    <th className="text-center px-4 py-3 text-xs font-semibold text-gray-400 uppercase">Restock (disarankan)</th>
                  </tr>
                </thead>
                <tbody>
                  {data.productForecasts.map(p => (
                    <tr key={p.productId} className="border-b border-gray-800/50 hover:bg-gray-800/30">
                      ...
                      <td className="px-4 py-3.5 text-center text-gray-300">{p.dailyVelocity}/hr</td>
                      <td className="px-4 py-3.5 text-center text-gray-300 text-xs">{p.daysUntilStockout}</td>
                      <td className="px-4 py-3.5 text-center">
                        <span className={`text-xs font-semibold px-2 py-1 rounded-full border ${riskColors[p.riskLevel]}`}>
                          {p.riskLevel}
                        </span>
                      </td>
                      <td className="px-4 py-3.5 text-center">
      ...
      ) : <p className="text-gray-400 text-sm">Gagal memuat data prediksi.</p>}
    </div>
  );
}
